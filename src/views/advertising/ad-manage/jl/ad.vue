      ...
      <div class="select-container list_filter">
        <div class="left">
          ...
          <ShowCreate
            ...
            :activeHandle="manage_ad_handle"
            :error="has_error"
            @updateHandle="update_handle"
          ></ShowCreate>
          <ErrorMsg
            v-if="has_error && manage_ad_handle == 'show'"
            :Number="rule_error"
          ></ErrorMsg>
          <div
            class="left-right"
            v-if="manage_ad_handle == 'edit'"
          >
            <!-- 推送 -->
    ...
    >
      <main ref="table_main">
        <DataTable
          ...
          :is_edit="manage_ad_handle == 'edit'"
          :checked="checked_ad"
          @update_list="update_detail_table"
          @selected="update_selected($event, 'ad')"
          @changeStatus="change_status"
...
import AdBatchEdit from "./components/edit/ad-batch-edit";
...
export default {
  ...
  data() {
    return {
      request_titlelist_url: "/adpush/jl/ad/titlelist",
      currentPage: 1,
      pagesize: 10,
      total: 0,
      filter_form: {
      ...
      deep_into_keys: ["ad_name"], // 可切换到下一级
      can_edit_keys: [
        "ad_name",
        "budget",
        "bid",
  ...
  watch: {
    manage_ad_handle: {
      handler(val) {
        if (val == "show") {
          this.total_data[0].ad_name = this.total_header;
          this.total_data[0].operate = "";
        } else {
          this.total_data[0].operate = this.total_header;
          this.total_data[0].ad_name = "";
        }
      },
    },
  },
  computed: {
    ...
    set_select_data() {
      ...
      const not = ["2", "3", 2, 3];
      if (this.checked_ad.length) {
        this.checked_ad.forEach((item) => {
          const index = this.detail_data.findIndex((obj) => {
            return obj.media_ad_id == item;
          });
          if (index >= 0) {
            if (
              !not.includes(this.detail_data[index].rule_detail_status) ||
              this.detail_data[index].status != "AD_STATUS_DELETE"
            ) {
  ...
  methods: {
    ...
    format_filter() {
      let delivery_date = "";
      if (this.filter_form.delivery_date.length) {
        delivery_date = `${this.filter_form.delivery_date[0]}~${this.filter_form.delivery_date[1]}`;
      }
      let create_date = "";
      if (this.filter_form.create_date.length) {
        create_date = `${this.filter_form.create_date[0]}~${this.filter_form.create_date[1]}`;
      }
      ...
      const param = {
        ...
        pricing: this.filter_form.pricing.join(),
        inventory_type: this.filter_form.inventory_type.join(),
        smart_bid_type: this.filter_form.smart_bid_type.join(),
        delivery_content: this.filter_form.delivery_content.join(),
        auto_extend_enabled: this.filter_form.auto_extend_enabled.join(),
        auto_extend_targets: this.filter_form.auto_extend_targets.join(),
        rule_id: this.filter_form.rule_id.join(),
        ad_word: this.filter_form.word.replace(/(^\s*)|(\s*$)/g, ""),
    ...
    async get_table_data() {
      ...
      let url = "/adpush/jl/ad/getList";
      if (this.manage_ad_handle == "show") {
        url = "/adpush/jl/ad/getList";
      } else {
        url = "/adpush/jl/adedit/getList";
      }
      const res = await this.fetchCore(url, this.format_filter(), false);

      if (res && res.total) {
        this.$nextTick(() => {
          if (this.manage_ad_handle == "edit") {
            // this.update_ad_syncing(res.sync)
        ...
        if (res.total != 0) {
          this.total_data = [
            Object.assign(res.totallist, {
              operate: "",
              ad_name: `${this.$t("lang.public_total")}（${
                res.total
    ...
    update_filter(param) {
      ...
      this.update_ad_has_filter(param.state);
      this.filter_change();
    },
    add_filter() {
    ...
    filter_change(page = 1, state = true) {
      ...
      this.update_ad_filter(param);
      this.get_table_data();
      if (state) {
        this.update_selected([], "ad");
      }
    },
    // 查看或编辑
    update_handle(param) {
      ...
      this.$nextTick(() => {
        if (param == "show") {
          ...
          this.update_ad_has_filter(true);
        }
    ...
    async download_table() {
      ...
      if (times > full_year) {
        ...
        const url = "/adpush/jl/ad/download";
    ...
    handle_batch_push() {
      ...
      let param = {
        ...
        point:
          "<span>确定要推送这条计划的修改吗？系统将在<span style='color: #ff0000'>5分钟</span>之内完成修改，您将无法撤销本次推送。</span>",
        row_id: this.checked_ad,
      };
      ...
      if (this.checked_ad.length) {
        this.checked_ad.forEach((item) => {
          ...
          if (index >= 0) {
            ...
            if (row.rule_detail_status == 2) {
              param.point =
                "<span>勾选项中<span style='color: #ff0000'>包含已触发规则的计划</span>，系统已完成自动修改。确认要向巨量引擎平台推送这些计划的修改吗？系统将在10分钟之内完成修改，您将无法撤销本次推送。</span>";
            }
            // 存在异常
        ...
        param.form.media_ad_ids = result;
      }
      ...
      if (this.checked_ad.length == not_edit.length) {
        this.$message({
    ...
    handle_create_task(id = []) {
      if (id.length) {
        ...
        id.forEach((item) => {
          ...
          if (index >= 0) {
            if (!not.includes(this.detail_data[index].rule_detail_status)) {
              result.push(this.detail_data[index].str_ad_id);
            }
          }
        });
    ...
    async change_status(param, callback) {
      ...
      const url = "/adpush/jl/adedit/optstatus";
      const data = await this.fetchCore(url, param, false);
      if (data) {
    ...
    async save_one_edit(param, callback) {
      const url = "/adpush/jl/adedit/save";
      const data = await this.fetchCore(url, param, true);
      if (data) {
        callback(true, data.media_ad_id);
        // this.filter_form.order = "DESC";
        // this.filter_form.order_by = "utime";
        this.get_table_data();
        this.$message({
    ...
    async batch_save_ad(param, callback) {
    ...
    push_someone(row) {
      ...
      if (row.rule_detail_status == 0 || row.rule_detail_status == 1) {
        ...
        // 正常触发规则
        param.point =
          "<span>当前计划触发规则，系统已完成自动修改。确认要向巨量引擎平台推送这条计划的修改吗？系统将在<span style='color: #ff0000'>5分钟</span>之内完成修改，您将无法撤销本次推送。</span>";
      }

      this.$refs.PushDialog.show(param);
    },
    async save_push(param) {
      const url = "/adpush/jl/adedit/pushad";
      ...
      if (res) {
        ...
        if (res.success && res.error) {
          this.$message({
            message: `${res.success}条计划推送成功！${res.error}条计划推送失败！您可联系管理员寻求帮助！`,
            duration: 5000,
            offset: 3,
            type: "error",
          });
          ...
          this.$message({
            message: `${res.error}条计划推送失败！您可联系管理员寻求帮助！`,
            duration: 5000,
            offset: 3,
            type: "error",
          });
    ...
    async save_ignore(param) {
      const url = "/adpush/jl/adedit/ignore";
      const res = await this.fetchCore(url, param, false);
      if (res) {
        // this.filter_form.order = "DESC";
        // this.filter_form.order_by = "utime";
    ...
    async get_select_list() {
      ...
      const url = "/adpush/jl/ad/getFilterList";
      const param = {
        team_id: this.active_team_id,
        media_campaign_id: this.checked_campaign.join(),
        advertiser_id: this.checked_account.join(),
  ...
  mounted() {
    ...
    this.$eventHub.$on("createAdTask", (data) => {
  ...
  created() {
    if (this.ad_filter != null) {
      this.filter_form = _.cloneDeep(this.ad_filter);
