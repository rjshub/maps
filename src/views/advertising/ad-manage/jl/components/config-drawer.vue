  ...
  >
    ...
    <section class="drawer-main">
      <div class="main-left">
        ...
        >
          <el-tab-pane
            v-for="(item, index) in set_module_list"
            :key="index"
            :name="item[language]"
            :label="item[language]"
          ></el-tab-pane>
          ...
          >
            <div class="item-header">
              <span style="line-height: 18px">
                ...
                >
                  ...
                  <i
                    ...
                    v-if="item.hasOwnProperty(`desc_${language}`) && item[`desc_${language}`] != ''"
                  ></i>
                </maps-tooltip>
              </span>
              ...
              <el-checkbox
                ...
                @change="all_check_change(item_index)"
              >{{$t('lang.public_check_all')}}</el-checkbox>
            </div>
            <div class="item-child">
              <div
                class="child not-icon-child"
                v-for="(child, child_index) in item.children"
                :key="child_index"
              >
                <maps-tooltip
                  :text="set_child_desc(child)"
                  align="left"
                >
                  <el-checkbox
                    v-model="child.mychecked"
                    :disabled="child.disable"
                    @change="title_check_change(item_index,child_index)"
                  >{{child[language]}}</el-checkbox>
                </maps-tooltip>
              </div>
            </div>
          </div>
      ...
      <div class="main-right">
        <div class="right-title">
          ...
          <el-button
            ...
            @click="clear_all"
            size="mini"
          >{{$t('lang.public_clear_all')}}</el-button>
        </div>
        <draggable
          class="grag-warp"
          v-model="result_list"
        ...
        >
          ...
          >
            <div>
              <i
                :class="special_key.includes(result.key)? '':'maps-icon-drag'"
                class="iconfont"
              ></i>
              {{result[language]}}
            </div>
            <span
              v-if="!set_forbidden_delete.includes(result.key)"
              @click="clear(result)"
            >&times</span>
          </div>
        </draggable>
...
export default {
  ...
  computed: {
          ...
          if (item.children && item.children.length) {
            ...
            item.children.forEach((child) => {
              if (
                ...
                child[this.language].toLowerCase() == word.toLowerCase()
              ) {
    ...
    set_forbidden_delete() {
      return this.special_key.concat(this.forbidden_delete_key);
    },
  },
  methods: {
    show() {
    ...
    title_check_change(father_index, item_index) {
      ...
      const index = this.titleList.findIndex((obj) => {
        return (
          this.set_title_list[father_index][this.language] == obj[this.language]
        );
      });
      if (index >= 0) {
        ...
        const param = {
          parent: this.titleList[index].key,
          ...this.set_title_list[father_index].children[item_index],
        };
    ...
    clear(param) {
      const parentIndex = this.titleList.findIndex((obj) => {
        return obj.zh === param.parent;
      });
      const childIndex = this.titleList[parentIndex].children.findIndex(
        (obj) => {
    ...
    async handle_save_config() {
      // if (this.save_config) {
      const url = `/adpush/jl/${this.type}/updatetitle`;
