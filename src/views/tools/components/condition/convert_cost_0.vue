<template>
    <div class="convert_cost-condition">
        <el-form :model="form"
            :rules="rules"
            ref="convert_costForm"
            label-width="0"
...
export default {
  ...
  methods: {
    ...
    data_range_change(param, update = true) {
      if (update) {
        this.refresh();
        this.$refs.convert_costForm.clearValidate();
      }
      ...
      if (param == 1) {
        ...
        if (this.form.compare == 6) {
          //介于
          this.module_list = _.cloneDeep(formulaConfig.convert_cost_1_6);
        } else {
          // 其他
          this.module_list = _.cloneDeep(formulaConfig.convert_cost_1_X);
        }
      } else if (param == 3) {
        if (this.form.compare == 6) {
          //介于
          this.module_list = _.cloneDeep(formulaConfig.convert_cost_3_6);
        } else {
          // 其他
          this.module_list = _.cloneDeep(formulaConfig.convert_cost_3_X);
        }
      }
    },
    ...
    validator_form() {
      let state = true;
      this.$refs.convert_costForm.validate((valid) => {
        if (valid) {
          state = true;
